...
typedef struct s_stub_config {} stub_config_t;

// 
typedef struct s_string_fixed_config_t {
    uint8_t count;
    uint8_t default_idx;
    const char ** values;
} string_fixed_config_t;
// 
typedef struct s_udword_factor_config_t {
    uint32_t max;
    uint32_t min;
    uint32_t step;
...
// 
typedef struct s_ubyte_simple_config_t {
...
} menu_node_config_data_t;

typedef struct s_menu_node_config_t {
    menu_id_t id;
